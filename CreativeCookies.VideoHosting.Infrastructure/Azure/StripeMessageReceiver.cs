...
namespace CreativeCookies.VideoHosting.Infrastructure.Azure
{
    public class StripeMessageReceiver : IHostedService
    {
        ...
        public StripeMessageReceiver(IBackgroundJobClient backgroundJobClient, IUsersRepository userRepo, StripeWebhookSigningKeyWrapper wrapper, 
            IStripeProductsService stripeProductsService, ISubscriptionPlanService subscriptionPlanService, IConnectAccountsService connectAccountsService,
            ICheckoutService checkoutService, IConfiguration configuration, ILogger<StripeMessageReceiver> logger)
        {
            _logger = logger;
            ...
            _connectAccountsSrv = connectAccountsService;
                ...
                else if (stripeEvent.Type == Events.CheckoutSessionCompleted)
                {
                    try
                    {
                        ...
                        if (checkoutSession.Mode.Equals("payment"))
                        {
                            ...
                            var res = _userRepo.ChangeSubscriptionDatesUTC(checkoutSession.CustomerId, subscriptionStartDate, subscriptionEndDate);
