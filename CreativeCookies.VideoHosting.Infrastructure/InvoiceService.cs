...
namespace CreativeCookies.VideoHosting.Infrastructure
{
    public class InvoiceService : IInvoiceService
    {
        ...
        public async Task<Attachement> GenerateInvoicePdf(decimal amount, string currency, InvoiceAddressDto buyerAddress, MerchantDto merchant)
        {
            ...
            issuerYPos += lineSpacing;
            gfx.DrawString("VAT EU: PL1231479701", regularFont, blackBrush, new XRect(leftMargin, issuerYPos, columnWidth - leftMargin, lineSpacing), XStringFormats.TopLeft);
        ...
        public async Task<Attachement> GenerateInvoicePdf2(decimal amount, string currency, InvoiceAddressDto buyerAddress, MerchantDto merchant)
        {            
            _logger.LogInformation($"Starting invoice generation to: {buyerAddress.FirstName} {buyerAddress.LastName}");
            var invoiceNumber = await _invoiceNumsRepository.GetNewNumber();
            var merchantHouseNoLine = $"{merchant.HouseNo} " + (merchant.AppartmentNo != null ? $"lok. {merchant.AppartmentNo}" : "");
            ...
            gfx.DrawString("Data wystawienia:", regularFont, blackBrush, new XRect(leftMargin + columnWidth, issuerYPos, columnWidth, page.Height));
            gfx.DrawString(DateTime.UtcNow.ToShortDateString(), regularFont, blackBrush, new XRect(leftMargin + columnWidth, issuerYPos + lineSpacing, columnWidth, page.Height));

            gfx.DrawString("Data sprzedaży:", regularFont, blackBrush, new XRect(leftMargin + (columnWidth * 2), issuerYPos, columnWidth, page.Height));
            gfx.DrawString(DateTime.UtcNow.ToShortDateString(), regularFont, blackBrush, new XRect(leftMargin + (columnWidth * 2), issuerYPos + lineSpacing, columnWidth, page.Height));
            ...
            gfx.DrawString("Netto", regularFont, blackBrush, new XRect(leftMargin + 2 * columnSpacing, tableStartY, columnSpacing, page.Height), XStringFormats.TopLeft);
            gfx.DrawString("VAT", regularFont, blackBrush, new XRect(leftMargin + 3 * columnSpacing, tableStartY, columnSpacing, page.Height), XStringFormats.TopLeft);
            gfx.DrawString("Brutto", regularFont, blackBrush, new XRect(leftMargin + 4 * columnSpacing, tableStartY, columnSpacing, page.Height), XStringFormats.TopLeft);
